...
def transform_image(image,
    gray=True, bbox=None, dims=None, stdiz=False, normlz=False, thresh=None, circle_mask_radius=None, **kwargs
):
    ''' Transform image (e.g. before applying neural network). '''
    
    if gray and image.ndim == 3 and image.shape[2] == 3:
    ...
    if thresh is not None:
        image = threshold_image(image, thresh)[..., np.newaxis]
...
def augment_image(image,
    ...
    ''' Augment image (e.g., before applying neural network). '''
...
def apply_circle_mask(image, radius=110, circle_mask_offset=[0, 0]):
    ...
    mask = cv2.circle(np.ones((hh, ww), dtype=image.dtype), (wc, hc), radius, 0, -1)
    return np.where(mask == 1, 0, image)
...
def random_zoom_image(x, zoom_range, fill_mode='nearest', cval=0.):
    ...
    zx, zy = (1, 1) if zoom_range[0] == 1 and zoom_range[1] == 1 else np.random.uniform(zoom_range[0], zoom_range[1], 2)
